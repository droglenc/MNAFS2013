...
\begin{figure}[h!]
  \centering
    ...
    \caption{Example of the RStudio layout with the \emph{Script Editor} in the upper-left panel, \emph{Console} in the lower-left panel, the \emph{Workspace} tab shown in the upper-right panel, and the \emph{Plot} tab shown in the lower-right panel.}
  \label{fig:RStudioLayout}
\end{figure}

The upper-right panel contains two tabs -- \emph{Workspace} and \emph{History}.  Many items listed under the \emph{Workspace} tab can be double-clicked to open them for viewing as a tab in the \emph{Script Editor}.  The \emph{History} tab simply shows all of the commands that you have submitted to the \emph{Console}.
