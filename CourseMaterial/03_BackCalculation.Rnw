...
\begin{document}
  \titleRHO{Back-Calculation}{MN AFS}{Dec}
...
\section*{Talcot Lake (MN) Black Crappie}
<<>>=
...
\newpage
\section*{Scale-Length and Length-Scale Relationships} 
<<>>=
...
<<scalelen1, par2=TRUE, fig.width=8, fig.height=4, out.width='0.9\\linewidth', fig.show='hold'>>=
...
\clearpage
\section*{Reshape to One-Measurement-Per-Line Format}
<<>>=
...
\section*{Fraser-Lee Back-Calculation} 
<<>>=
blcT2 <- within(blcT2, lenFL <- (anu/radcap)*(lencap-c)+c)
view(blcT2)
@

\section*{Scale Proportional Hypothesis (SPH) Back-Calculation} 
<<>>=
blcT2 <- within(blcT2, lenSPH <- (anu/radcap)*(lencap+(a/b))-(a/b))
view(blcT2)
@

\section*{Summaries of Back-Calculated Lengths} 
<<>>=
( sum1 <- Summarize(lenFL~age,data=blcT2,digits=2) )
( sum2 <- Summarize(lenFL~age*agecap,data=blcT2,digits=2) )
@

<<bcsum, fig.width=4, fig.height=4, out.width='0.45\\linewidth'>>=
...
swvCode("03_BackCalculation.Rnw")
@
